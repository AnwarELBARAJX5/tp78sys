...
/************************************************************
 rechercher et renvoyer le numéro d'un nom dans le répertoire.
...
int find_inode(const char* name) {
    ...
    return ((index < 0) ? -1 : directory.files[index].adr_inode);
}
...
int add_inode (const char* name, int inode) {
    ...
    int old_index = find_name(name);
    if (old_index >= 0) {
        int old_inode = directory.files[old_index].adr_inode;
        directory.files[old_index].adr_inode = inode;
    ...
    for(int j = 0; j < DIR_SIZE; j++) {
        if (directory.files[j].adr_inode == 0) {
            ...
            strcpy(directory.files[j].name, name);
            write_dir_block(& directory);
            return (0);
        }
    }
...
void delete_inode (const char* name) {
    int index = find_name(name);
    if (index >=0) {
        directory.files[index].adr_inode = 0;
        strcpy(directory.files[index].name, "");
        write_dir_block(& directory);
    }
}
